...
# Load in files
cryptic = fread(paste0(ROOTDIR, "data/CRyPTIC_reuse_table_20231107.csv"))
...
```
There are `r length(ids_in_common)` biosample IDs in common between mykrobe and cryptic training datasets. These are all site 6/Italian samples that were submitted to cryptic.  
...
colnames_order = c("ena_run", "ena_sample", "ena_project", drugs)
...
cryptic_edited = cryptic %>% 
  select(ends_with("TYPE"), c("ENA_RUN", "ENA_SAMPLE")) %>%
...
content <- c("Training data: 25033 samples, file = training_data_20231122.tsv",
             "Validation data: 894 samples, file = validation_set_20231110.pass.tsv",
